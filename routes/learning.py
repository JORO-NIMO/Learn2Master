...
def submit_practical_evidence(outcome_id):
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, (learner_id, outcome_id, title, description, file_path))
    conn.execute("""
...
def submit_assessment(assessment_id):
    ...
    assessment = conn.execute("""
        SELECT assessments.*, lessons.outcome_id, lessons.lesson_id, lo.outcome_name, lo.mastery_threshold
        FROM assessments
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, (learner_id, assessment["lesson_id"], assessment["outcome_id"], rec["reason"], rec["type"]))
